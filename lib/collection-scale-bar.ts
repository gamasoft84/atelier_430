...
export interface ScaleBarChoice {
  ...
  segmentCm: 50 | 100
  /** Texto corto para la leyenda. */
  label: string
}

/**
 * Elige entre una marca de 50 cm o 1 m según el zoom efectivo en pantalla,
 * para que la barra gráfica siga siendo legible.
 */
export function pickScaleBarSegment(effectivePxPerCm: number): ScaleBarChoice {
  if (!Number.isFinite(effectivePxPerCm) || effectivePxPerCm <= 0) {
    return { segmentCm: 100, label: "1 m" }
  }
  const px100 = 100 * effectivePxPerCm
  const px50 = 50 * effectivePxPerCm
  if (px100 >= 56) return { segmentCm: 100, label: "1 m" }
  if (px50 >= 40) return { segmentCm: 50, label: "50 cm" }
  return px100 >= px50 ? { segmentCm: 100, label: "1 m" } : { segmentCm: 50, label: "50 cm" }
}
